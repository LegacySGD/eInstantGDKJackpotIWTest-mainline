...
public class GDKJackpotIWTestWLAEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(GDKJackpotIWTestWLAEngineManagerTest.class);
		logger.setLevel(Level.DEBUG);
		gameParams = XMLUtil.parseResource("paymodel/GDKJackpotIWTestIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/GDKJackpotIWTestIW/InitOutcome.xml").getDocumentElement();
			...
			throws Exception
	{
		...
		assertSettledPendingPayout(outcome, 0, 50, 0);
		...
		throws Exception
	{
		...
		playerInput = new PlayerInputBuilder()
				.appendActionInput(buyAction)
				.appendWagerInput(new Wager("Wager1", weg))
				.appendJSONInput("{'testData':0}").build();
		Node outcome = manager.play(InputUtil.buildInput(gameParams, previousOutcome, playerInput));
		logger.debug(outcome);
		
		assertEquals("Scenario", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, 0, weg, 0);
		...
		assertJsonContent(
				...
				odeRevealDataOutcomeWrapper);
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, weg, 0, 100);
	}
			...
			throws Exception
	{
		...
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, weg, 0, 0);
	}
	...
	@Test
	public void test_buy_2000x_JackpotPrize_RevealThreeTimes()
		throws Exception

		{
			...
			// Scenario
			playerInput = new PlayerInputBuilder().appendActionInput("play").build();
			TestODEProxy.setJsonResponse("{'prizeValue':994156,'prizeDivision':1}");
			outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
			outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
			logger.debug(outcome);
	...
	@Test
	public void test_try_2000x_JackpotPrize()
			throws Exception
	{
		int pricePoint = 2000;
		Element playerInput;
